...
def dist_prep(input_scale, inv_num, day_it_inv, ft_sample_inv):
    ...
    if day_it_inv == 0:
        while True:
            ft_sample_inv = math.ceil(input_scale * np.random.weibull(4))        # failure time is always the real EUR
...
def inv_simulation (inv_num, input_scale, input_c_p, input_c_f, na_days_pm, na_days_cm, simulation_end_days, simulation_step_days, day, day_it,
    ...
    [x_values, y_pdf_values, y_cdf_values, ft_sample_inv, distance_ft] = dist_prep(input_scale, inv_num,
                                                                                   day_it_inv, ft_sample_inv)
